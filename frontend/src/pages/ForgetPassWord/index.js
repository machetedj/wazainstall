...
const handleSendEmail = async (values) => {
  ...
  try {
    ...
    if (response.data.status === 404) {
      ...
      toast.success(i18n.t("Email ¡Enviado con éxito!"));
    }
  } catch (err) {
  ...
  const handleResetPassword = async (values) => {
    ...
    if (newPassword === confirmPassword) {
      try {
        ...
        setError(""); // Borre el error si no hay ningún error
  ...
  const UserSchema = Yup.object().shape({
    ...
    newPassword: isResetPasswordButtonClicked
      ? Yup.string()
          ...
          .matches(
            passwordRegex,
            "Su contraseña debe tener al menos 8 caracteres, incluyendo una letra mayúscula, una letra minúscula y un número."
          )
      : Yup.string(), // Sen validación si no se restablece la contraseña
    confirmPassword: Yup.string().when("newPassword", {
      is: (newPassword) => isResetPasswordButtonClicked && newPassword,
      then: Yup.string()
        .oneOf([Yup.ref("newPassword"), null], "Las contraseñas no coinciden")
        .required("Campo obligatorio"),
      otherwise: Yup.string(), // No hay validación si no se restablece la contraseña
    }),
  });
                ...
                <Grid container justifyContent="flex-end">
                  <Grid item>
                    <Link
                      ...
                      to="/signup"
                    >
                      {i18n.t("¿No tienes una cuenta? ¡Registro!")}
                    </Link>
                  </Grid>
                </Grid>
                {error && (
