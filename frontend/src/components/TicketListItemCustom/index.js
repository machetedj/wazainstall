...
const useStyles = makeStyles((theme) => ({
  ...
  {/*DURKY INSERIDO O dentro do const handleChangeTab*/}
  ...
  useEffect(() => {
    ...
    const updateLastInteractionLabel = () => {
      ...
      // Programar la próxima actualización después de 30 segundos
      setTimeout(updateLastInteractionLabel, 30 * 1000);
    };

    // Inicializando la primera actualización
    updateLastInteractionLabel();

  }, [ticket]); // Ejecutando solo una vez al montar el componente
  ...
  const handleReopenTicket = async (id) => {
    ...
    const handleAcepptTicket = async (id) => {
        ...
        const msg = `{{ms}} *{{name}}*, mi nombre es *${user?.name}* y ahora procederé con tu ¡servicio!`;
        const message = {
            read: 1,
            fromMe: true,
            mediaUrl: "",
  ...
  return (
    <React.Fragment key={ticket.id}>
      ...
      >
        <Tooltip arrow placement="right" title={ticket.queue?.name?.toUpperCase() || "Sin Depart"} >
        ...
        <ListItemSecondaryAction>
          ...
          secondary={
            <span className={classes.contactNameWrapper}>
              ...
              <MarkdownWrapper> Ubicación</MarkdownWrapper> : 
            ...
            {ticket.lastMessage && verpreview ? (
              <>
                {ticket.lastMessage.includes('VCARD') ? (
                  <MarkdownWrapper>Nuevo contacto recibido</MarkdownWrapper>
                ) : ticket.lastMessage.includes('data:image') ? (
                  <MarkdownWrapper>Ubicación recebida</MarkdownWrapper>
                ) : (
                  <MarkdownWrapper>
              ...
              <MarkdownWrapper>---</MarkdownWrapper>
                ...
                <span className={classes.secondaryContentSecond} >
                  ...
                  <Badge style={{ backgroundColor: ticket.queue?.color || "#7c7c7c" }} className={classes.connectionTag}>{ticket.queue?.name?.toUpperCase() || "Sin Depart."}</Badge>
                </span>
        ...
        <span className={classes.secondaryContentSecond} >
          {ticket.status === "pending" && (
            <ButtonWithSpinner
              ...
              size="small"
              loading={loading}
			  //DURKY INSERIDO O handleChangeTab
