...
const ImportContactsService = async (companyId: number): Promise<void> => {
  ...
  try {
    ...
    logger.error(`No se pudieron obtener los contactos de WhatsApp desde el teléfono. Err: ${err}`);
  }
  ...
  fs.writeFile(afterFilePath, JSON.stringify(phoneContacts, null, 2), (err) => {
    if (err) {
      logger.error(`No se pudieron escribir los contactos en el archivo: ${err}`);
      throw err;
    }
  });
    ...
    phoneContactsList.forEach(async ({ id, name, notify }) => {
      ...
      if (existingContact) {
        ...
        try {
          ...
          logger.warn(
            `No se pudieron obtener los contactos de WhatsApp desde el teléfono. Err: ${error}`
          );
        }
      }
    });
