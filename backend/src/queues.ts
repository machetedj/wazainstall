...
async function handleVerifyCampaigns(job) {
  ...
  for (let campaign of campaigns) {
    try {
      ...
      logger.info(
        `Campaña enviada a la linea de procesamiento: Campanha=${campaign.id}, Delay Inicial=${delay}`
      );
      campaignQueue.add(
        "ProcessCampaign",
        {
...
async function sleep(seconds) {
  logger.info(
    `dormir ${seconds} segundos comenzaron: ${moment().format("HH:mm:ss")}`
  );
  return new Promise(resolve => {
    setTimeout(() => {
      logger.info(
        `dormir ${seconds} segundos terminados: ${moment().format(
          "HH:mm:ss"
        )}`
      );
      resolve(true);
